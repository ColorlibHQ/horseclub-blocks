 ...
 * `[horseclub_form]` survives the round trip because a shortcode is
...
const HORSECLUB_SETUP_FLAG = 'horseclub_front_page_created';
 ...
 */
function horseclub_starter_pages() {
	return array(
		'home'     => array(
			'title'    => __( 'Home', 'horseclub' ),
			'pattern'  => 'horseclub/page-home',
			'template' => 'page-no-title',
		),
		'about'    => array(
			'title'    => __( 'About', 'horseclub' ),
			'pattern'  => 'horseclub/page-about',
			'template' => '',
		),
		'services' => array(
			'title'    => __( 'Services', 'horseclub' ),
			'pattern'  => 'horseclub/page-services',
			'template' => '',
		),
		'training' => array(
			'title'    => __( 'Training', 'horseclub' ),
			'pattern'  => 'horseclub/page-training',
			'template' => '',
		),
		'events'   => array(
			'title'    => __( 'Events', 'horseclub' ),
			'pattern'  => 'horseclub/page-events',
			'template' => '',
		),
		'pricing'  => array(
			'title'    => __( 'Pricing', 'horseclub' ),
			'pattern'  => 'horseclub/page-pricing',
			'template' => '',
		),
		'blog'     => array(
			'title'    => __( 'Blog', 'horseclub' ),
			'pattern'  => '',
			'template' => '',
		),
		'contact'  => array(
			'title'    => __( 'Contact', 'horseclub' ),
			'pattern'  => 'horseclub/page-contact',
			'template' => '',
		),
	);
}
 ...
 * overwrite the first.
 */
function horseclub_create_front_page() {
	if ( get_option( HORSECLUB_SETUP_FLAG ) ) {
		return;
	}

	// Only on a site that has not been built yet: someone activating Horseclub
		...
		array(
			...
			'exclude'        => array_filter(
				array(
					...
					horseclub_page_exists( 'sample-page' ),
				)
			),
		)
	);
	...
	if ( count( $existing ) > 1 ) {
		update_option( HORSECLUB_SETUP_FLAG, 'skipped: site already had pages', false );
		return;
	}
	...
	foreach ( horseclub_starter_pages() as $slug => $page ) {
		if ( horseclub_page_exists( $slug ) ) {
			continue;
		}

		$content = '';
		if ( $page['pattern'] ) {
			$content = horseclub_pattern_content( $page['pattern'] );
			if ( '' === $content ) {
				continue;
			}
		}
	...
	horseclub_create_primary_menu( $created );
	...
	if ( $created ) {
		update_option( HORSECLUB_SETUP_FLAG, gmdate( 'c' ), false );
	}
}
add_action( 'after_switch_theme', 'horseclub_create_front_page' );
 ...
 * @return int
 */
function horseclub_page_exists( $slug ) {
	$ids = get_posts(
		array(
			'name'           => $slug,
			'post_type'      => 'page',
 ...
 */
function horseclub_create_front_page_retry() {
	if ( get_option( HORSECLUB_SETUP_FLAG ) || ! current_user_can( 'edit_theme_options' ) ) {
		return;
	}

	horseclub_create_front_page();
}
add_action( 'admin_init', 'horseclub_create_front_page_retry' );
...
function horseclub_pattern_content( $name, $seen = array() ) {
	...
	return (string) preg_replace_callback(
		...
		static function ( $matches ) use ( $seen ) {
			...
			$nested = horseclub_pattern_content( $attributes['slug'], $seen );
 ...
 */
function horseclub_create_primary_menu( $pages ) {
	if ( ! $pages ) {
		return;
	}

	$order = array( 'home', 'about', 'services', 'training', 'events', 'pricing', 'blog', 'contact' );
	$items = '';
	...
	wp_insert_post(
		array(
			'post_title'   => __( 'Primary', 'horseclub' ),
			'post_name'    => 'primary',
