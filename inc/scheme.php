 ...
 * site owner chose.
 *
 * It **lifts the active palette rather than replacing it**. A Horseclub site set to
 ...
 *
 *     add_filter( 'horseclub_enable_dark_mode', '__return_false' );
 *
 * @return bool
 */
function horseclub_dark_mode_enabled() {
	return (bool) apply_filters( 'horseclub_enable_dark_mode', true );
}
 ...
 */
function horseclub_scheme_boot_script() {
	if ( ! horseclub_dark_mode_enabled() ) {
		return;
	}
	?>
<script>
( function () {
	try {
		var stored = localStorage.getItem( 'horseclub-scheme' );
		var system = window.matchMedia( '(prefers-color-scheme: dark)' ).matches;
		var dark = stored ? 'dark' === stored : system;
		document.documentElement.classList.toggle( 'horseclub-dark', dark );
...
add_action( 'wp_head', 'horseclub_scheme_boot_script', 1 );

/**
 * The stylesheet and the toggle's behaviour.
 */
function horseclub_scheme_assets() {
	if ( ! horseclub_dark_mode_enabled() ) {
		return;
	}
	...
	wp_localize_script(
		...
		'horseclubScheme',
		array(
			'toDark'  => __( 'Switch to dark mode', 'horseclub' ),
			'toLight' => __( 'Switch to light mode', 'horseclub' ),
		)
	);
}
add_action( 'wp_enqueue_scripts', 'horseclub_scheme_assets' );

/**
 * Turn any button carrying `horseclub-scheme-toggle` into the switch.
 *
 ...
 * @return string
 */
function horseclub_scheme_toggle_markup( $content, $block ) {
	$classes = isset( $block['attrs']['className'] ) ? $block['attrs']['className'] : '';
	if ( false === strpos( $classes, 'horseclub-scheme-toggle' ) ) {
		return $content;
	}
	...
	$content = str_replace(
		'<a ',
		'<a role="button" aria-pressed="false" aria-label="' . esc_attr__( 'Switch to dark mode', 'horseclub' ) . '" ',
		$content
	);

	return $content;
}
add_filter( 'render_block_core/button', 'horseclub_scheme_toggle_markup', 10, 2 );
