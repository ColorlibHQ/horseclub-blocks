 ...
 * call our filter.
 */
const HORSECLUB_UPDATE_HOST     = 'updates.colorlib.com';
const HORSECLUB_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/horseclub.json';
const HORSECLUB_UPDATE_CACHE    = 'horseclub_update_response';
 ...
 * @return bool
 */
function horseclub_updates_enabled() {
	/**
	 * Filters whether Horseclub checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'horseclub_check_for_updates', true );
}
 ...
 * @return array
 */
function horseclub_update_payload() {
	$payload = array(
		'theme'      => 'horseclub',
		'version'    => HORSECLUB_VERSION,
	 ...
	 */
	return apply_filters( 'horseclub_update_payload', $payload );
}
 ...
 * @return array|null
 */
function horseclub_fetch_update() {
	if ( ! horseclub_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( HORSECLUB_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( horseclub_update_payload(), HORSECLUB_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => horseclub_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( HORSECLUB_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( HORSECLUB_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function horseclub_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'horseclub' !== $theme_stylesheet ) {
		return $update;
	}

	$release = horseclub_fetch_update();

	if ( ! $release || version_compare( $release['version'], HORSECLUB_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => HORSECLUB_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/horseclub/',
...
add_filter( 'update_themes_' . HORSECLUB_UPDATE_HOST, 'horseclub_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function horseclub_flush_update_cache() {
	delete_site_transient( HORSECLUB_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'horseclub_flush_update_cache' );
add_action( 'after_switch_theme', 'horseclub_flush_update_cache' );

/**
 * Explain the update check on the Themes screen.
 *
 * A site owner should not have to read the source to find out what leaves
 * their server.
 */
function horseclub_updates_notice() {
	...
	$payload = horseclub_update_payload();

	// Horseclub ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Horseclub checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'horseclub' );
	?>
	<p class="description horseclub-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( HORSECLUB_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>horseclub_check_for_updates</code>'
		);
		?>
	</p>
	<style>.horseclub-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'horseclub_updates_notice' );
 ...
 * @return string
 */
function horseclub_update_user_agent() {
	return 'Horseclub/' . HORSECLUB_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function horseclub_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = horseclub_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'horseclub_update_request_args', 10, 2 );
