...
( function () {
	...
	var KEY = 'horseclub-scheme';
	var root = document.documentElement;
	var strings = window.horseclubScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'horseclub-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'horseclub-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.horseclub-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'horseclub-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
