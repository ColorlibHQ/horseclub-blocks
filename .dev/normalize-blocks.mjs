...
// HORSECLUB_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.HORSECLUB_PATTERNS
	? resolve( process.env.HORSECLUB_PATTERNS )
	: resolve( here, '../patterns' );

const url = process.env.WP_URL || 'http://127.0.0.1:9494';
...
// `<?php ... ?>` -> `HCPHP0`, and back again after the round trip.
...
function restore( content, snippets ) {
	return content.replace( /HCPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
