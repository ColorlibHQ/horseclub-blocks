 ...
 *
 *   WP_URL=http://127.0.0.1:9494 node .dev/editor-check.mjs
...
const url = ( process.env.WP_URL || 'http://127.0.0.1:9494' ).replace( /\/$/, '' );
const shots = join( resolve( dirname( fileURLToPath( import.meta.url ) ) ), 'editor-shots' );
const only = process.env.PATTERNS ? process.env.PATTERNS.split( ',' ).map( ( s ) => 'horseclub/' + s ) : null;
...
const patterns = await page.evaluate( async () => {
	...
	return all.filter( ( p ) => 0 === p.name.indexOf( 'horseclub/' ) ).map( ( p ) => ( { name: p.name, content: p.content } ) );
} );
...
for ( const pattern of patterns ) {
	...
	const result = await page.evaluate( () => {
		...
		const icons = [ ...doc.querySelectorAll( '[class*="horseclub-icon--"], .horseclub-header__square' ) ];
	...
	checked++;
	const file = join( shots, pattern.name.replace( 'horseclub/', '' ) + '.png' );
