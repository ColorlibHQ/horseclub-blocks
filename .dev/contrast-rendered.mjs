 ...
 * the strip.
 *
 *   WP_URL=... HORSECLUB_PATHS=/,/about/ node .dev/contrast-rendered.mjs
 *   HORSECLUB_DARK=1 WP_URL=... node .dev/contrast-rendered.mjs
 *
 * @package Horseclub
 */
...
const paths = ( process.env.HORSECLUB_PATHS || process.env.HORSECLUB_URL || '/,/about/,/services/,/training/,/events/,/pricing/,/contact/,/blog/,/spring-league-round-one/,/category/horse-care/,/?s=horse,/no-such-page/' ).split( ',' ).filter( Boolean );
...
if ( process.env.HORSECLUB_PALETTE ) {
	await page.addStyleTag( { content: await paletteCss( process.env.HORSECLUB_PALETTE ) } );
	await page.waitForTimeout( 200 );
}

// HORSECLUB_DARK=1 measures the same page with dark mode on, which is where the
// palette is lifted rather than replaced and is the likeliest place for a
// pairing to fall under AA.
if ( process.env.HORSECLUB_DARK ) {
	await page.evaluate( () => {
		document.documentElement.classList.add( 'horseclub-dark' );
	} );
...
const candidates = await page.evaluate( () => {
	...
	document.querySelectorAll( 'h1,h2,h3,h4,h5,h6,p,li,a,dt,dd,summary,label,button,span' ).forEach( ( el, i ) => {
		...
		el.setAttribute( 'data-hc-cr', String( i ) );
		out.push( {
			i,
			need: large ? 3 : 4.5,
...
for ( const c of candidates ) {
	...
	onPhotos++;
	const handle = await page.$( `[data-hc-cr="${ c.i }"]` );
...
console.log( `\n${ paths.length } pages, ${ measuredTotal } text nodes, all at AA${ process.env.HORSECLUB_DARK ? ' (dark mode)' : '' }` );
