 ...
 *
 *   WP_URL=https://colorlibhub.com/horseclub node .dev/button-boundary.mjs
 *   HORSECLUB_DARK=1 WP_URL=… node .dev/button-boundary.mjs
 *   HORSECLUB_PATHS=/,/about/ WP_URL=… node .dev/button-boundary.mjs
 *
...
const base = ( process.env.WP_URL || 'http://127.0.0.1:9494' ).replace( /\/$/, '' );
const dark = process.env.HORSECLUB_DARK === '1';
const paths = ( process.env.HORSECLUB_PATHS || '/,/about/,/services/,/training/,/events/,/pricing/,/contact/,/blog/,/spring-league-round-one/,/category/horse-care/,/?s=horse,/no-such-page/' ).split( ',' );
...
if ( process.env.HORSECLUB_PALETTE ) {
	...
	const buttons = await page.evaluate( () => {
		return [ ...document.querySelectorAll( '.wp-block-button__link, .wp-element-button' ) ]
			...
			.filter( ( el ) => el.offsetWidth > 0 && ! el.closest( '.horseclub-scheme-toggle' ) )
			.map( ( el, i ) => {
				el.setAttribute( 'data-hc-bb', String( i ) );
	...
	for ( const b of buttons ) {
		...
		if ( b.onPhoto ) {
			onPhotos++;
			const handle = await page.$( `[data-hc-bb="${ b.i }"]` );
