 ...
 *
 * Measuring the DOCUMENT (`scrollWidth > innerWidth`) catches a page you can
 * scroll sideways and nothing else. A form whose inputs are 34px wider than
 * their grid cell — `width: 100%` plus padding on a content-box control — leaves
 * the document exactly as wide as the viewport, because the overflow happens
 * inside a panel in the middle of the page. That has shipped in a Colorlib
 * block theme before, and a person spotted it, not a check.
 *
 ...
 *
 *   WP_URL=https://example.com HORSECLUB_PATHS=/,/contact/ node .dev/overflow-check.mjs
 *
 * @package Horseclub
 */
...
const site = process.env.WP_URL || 'http://127.0.0.1:9494';
const paths = ( process.env.HORSECLUB_PATHS || '/,/about/,/services/,/training/,/events/,/pricing/,/contact/,/blog/,/spring-league-round-one/,/category/horse-care/,/?s=horse,/no-such-page/' ).split( ',' ).filter( Boolean );
const widths = ( process.env.HORSECLUB_WIDTHS || '1400,1024,768,390' ).split( ',' ).map( Number );
