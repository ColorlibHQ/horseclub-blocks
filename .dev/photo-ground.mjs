...
export async function sampleBehind( page, handle, pad = 6, { text = false } = {} ) {
	...
	await handle.evaluate( ( el, onlyText ) => {
		el.dataset.hcVisibility = el.style.visibility;
		el.dataset.hcColor = el.style.color;
		el.dataset.hcShadow = el.style.textShadow;
		if ( onlyText ) {
	...
	const restore = () => handle.evaluate( ( el ) => {
		el.style.visibility = el.dataset.hcVisibility || '';
		el.style.removeProperty( 'color' );
		el.style.removeProperty( 'text-shadow' );
		el.style.removeProperty( 'transition' );
		el.style.color = el.dataset.hcColor || '';
		el.style.textShadow = el.dataset.hcShadow || '';
		delete el.dataset.hcVisibility;
		delete el.dataset.hcColor;
		delete el.dataset.hcShadow;
	} );
 ...
 *
 *   HORSECLUB_PALETTE=colors-7-midnight node .dev/contrast-rendered.mjs
 */
export async function paletteCss( slug ) {
	...
	return `html:not(.horseclub-dark){${ all }} html.horseclub-dark{${ kept }}`;
}
