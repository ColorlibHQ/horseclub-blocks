#!/usr/bin/env python3
"""Block markup helpers for Horseclub's pattern generators.
...
def image(slug, alt, ratio=None, scale="cover", style=None, rounded=None, lightbox=False, width=None,
    ...
    if extra_class:
        ...
        cls.append(extra_class)
    if href:
        data["linkDestination"] = "custom"

    img_style = ' style="%s"' % ";".join(declarations) if declarations else ""
    url = theme_image(slug)
    img = '<img src="%s" alt="%s"%s/>' % (url, alt, img_style)
    if href:
        img = '<a href="%s">%s</a>' % (href, img)
    return (
        '<!-- wp:image%s -->\n'
        '<figure class="%s">%s</figure>\n'
        '<!-- /wp:image -->' % (attrs(data), classes(*cls), img)
    )


def theme_image(slug):
    """The URL of one of the theme's photographs, as PHP the pattern runs."""
...
def separator(style="horseclub-bar", align=None):
    data = {"className": "is-style-" + style}
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
          ...
          padding_y=None, anchor=None, focal=None):
    """A core/cover over one of the theme's photographs.

    `focal` is core's focalPoint, (x, y) from 0 to 1: the point of the
    photograph that stays in view when the cover crops it (object-position).
    ...
    if focal:
        data["focalPoint"] = {"x": focal[0], "y": focal[1]}
    if min_height:
    ...
    if extra_class:
        data["className"] = extra_class
    if padding_y:
        data["style"] = {"spacing": {"padding": {"top": sp(padding_y), "bottom": sp(padding_y)}}}
    ...
    if padding_y:
        css += ["padding-top:" + spc(padding_y), "padding-bottom:" + spc(padding_y)]
    if min_height:
        css.append("min-height:%d%s" % (min_height, unit))
    style_attr = ' style="%s"' % ";".join(css) if css else ""
    id_attr = ' id="%s"' % anchor if anchor else ""
    ...
    if gradient:
        ...
        # And 50 is core's default, for which it writes no step class at all.
        step = "" if dim in (0, 50) else " has-background-dim-%d" % (10 * round(dim / 10))
        span = ('<span aria-hidden="true" class="wp-block-cover__background '
                'has-%s-background-color%s has-background-dim"></span>' % (overlay, step))

    url = theme_image(slug)
    attr_json = attrs(data).replace('"HORSECLUB_IMAGE"', '"%s"' % url)

    # Core writes the focal point twice on the <img>: as the style and as
    # data-object-position, both "x% y%".
    position = ""
    if focal:
        pos = "%g%% %g%%" % (focal[0] * 100, focal[1] * 100)
        position = ' style="object-position:%s" data-object-fit="cover" data-object-position="%s"' % (pos, pos)

    return (
        '<!-- wp:cover%s -->\n'
        '<div%s class="%s"%s>'
        '<img class="wp-block-cover__image-background" alt="" src="%s"%s/>'
        '%s'
        '<div class="wp-block-cover__inner-container">\n%s\n</div></div>\n'
        '<!-- /wp:cover -->' % (attr_json, id_attr, classes(*cls), style_attr, url,
                               position or ' data-object-fit="cover"', span, inner)
    )
