#!/usr/bin/env python3
"""
Fail on any `horseclub-*` CSS selector that nothing in the theme emits.
...
`'horseclub-field--' . esc_attr( $name )` or JavaScript such as
`'horseclub-' + name` registers its literal prefix, and any selector starting
with that prefix is treated as live. Without that, `.horseclub-field--message`
...
TOKEN = re.compile(r"horseclub-[a-z0-9_-]+")
SELECTOR = re.compile(r"\.(horseclub-[a-z0-9_-]+)")
# A runtime prefix is the horseclub- token that ENDS a string literal and is then
# concatenated with a variable. It must not be required to START the literal:
#
#   class="horseclub-field horseclub-field--' . esc_attr( $name ) . '"
#
# begins at `class="`, so the first version of this pattern — which wanted a
# quote directly before the prefix — missed `horseclub-field--` and reported the
# live `.horseclub-field--message` as dead. Its own first run caught that.
PHP_PREFIX = re.compile(r"""(horseclub-[a-z0-9_-]*)['"]\s*\.\s*(?:[a-z_]+\s*\(\s*)?\$""")
# 'horseclub-' + name   /   `horseclub-${name}`
JS_PREFIX = re.compile(r"""(horseclub-[a-z0-9_-]*)(?:['"]\s*\+|\$\{)""")
...
REGISTERED = re.compile(r"""array\(\s*'(horseclub-[a-z0-9-]+)'\s*,\s*__\(""")
...
def main():
    ...
    # A bare `horseclub-` prefix (an id built as 'horseclub-' . $form) would make
    # every selector count as live and the whole check pass vacuously.
    prefixes = {p for p in prefixes if p.rstrip("-") != "horseclub"}
    ...
    print("%d horseclub-* selectors, %d emitted literally, dynamic prefixes: %s"
